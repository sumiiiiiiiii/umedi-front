...
function Condition(){
  const navigate = useNavigate();
	...
	const [conditionList, setConditionList] = useState(CONDITION_LIST);
	...
	const handleInputValueChanged = (e) => {
		...
		setValue(input);
		setConditionList(CONDITION_LIST.filter(condition => condition.toLowerCase().includes(input)));
	}

	const removeInputValue = () => {
		setValue("");
		setConditionList(CONDITION_LIST);
	}

	const handleConditionClicked = (condition) => {
		navigate(`/condition/${condition}/hospital`);
	}
				...
				<section className="section-area section-sp1 bg-white">
					<div className="container">
							...
							<div className="col-md-12 col-lg-5 col-xl-4 mb-30">
								...
								<div className="row">
									{
										isMatched && conditionList.length > 0 && conditionList.map((condition, idx) => (
											<div className="col-lg-6 col-sm-6" key={idx} onClick={() => handleConditionClicked(condition)}>
												<div className="feature-container feature-bx1 feature4 condition">
													...
													<div className="icon-content">
														<h4 className="ttr-title">{condition}</h4>
													</div>
												</div>
											</div>
											)
